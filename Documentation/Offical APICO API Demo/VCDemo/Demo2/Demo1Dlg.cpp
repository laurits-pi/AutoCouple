...
void CDemo1Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	if ((nID & 0xFFF0) == IDM_ABOUTBOX)
	{
		...
		{
	        Rtn = m_cDll.get_lib_ver(&ma[0], &mi1[0], &mi2[0]);
		    Rtn = m_cDll.get_sys_ver(&ma[1], &mi1[1], &mi2[1]);
			Rtn = m_cDll.get_card_ver(1, &nType, &ma[2], &mi1[2], &mi2[2]);
		}
		str.Format("板卡类型号     ：%d\n",nType);
	    sInfo = sInfo + str;
...
*/
	...
	{
       m_cDll.set_card_alm_logic(i+1,0);
	}
	for ( i=0;i<glTotalAxes;i++)
	{
		m_cDll.set_alm_logic(i+1,0);
        m_cDll.set_el_logic(i+1,0);
		m_cDll.set_org_logic(i+1,0);
	}
...
//	if(m_bBatch) m_cDll.set_batch_flag(1);	//批处理模式
//	else m_cDll.set_batch_flag(0);
//	m_cDll.set_maxspeed(1,8191);
	...
	if(m_iSelect==0)	//直线运动
	{
		...
		m_cDll.set_maxspeed(ChX,atoi(m_sXHighSpeed));
		m_cDll.set_maxspeed(ChX,atoi(m_sYHighSpeed));

		m_cDll.set_conspeed(ChX,atoi(m_sXLowSpeed));
		m_cDll.set_conspeed(ChX,atoi(m_sYLowSpeed));

		m_cDll.set_profile(ChX,atoi(m_sXLowSpeed),atoi(m_sXHighSpeed),atoi(m_sXAccel),atoi(m_sXAccel));
		m_cDll.set_profile(ChY,atoi(m_sYLowSpeed),atoi(m_sYHighSpeed),atoi(m_sYAccel),atoi(m_sYAccel));
		...
		if(m_iConFast==IDC_RADIO_CON)
		{
			if(m_iPVMove==IDC_RADIO_PMOVE)
			{
				...
				{
					m_cDll.con_pmove2(ChX,StepX,ChY,StepY);
				}
			}
			else
			{
				...
				{
					m_cDll.con_vmove2(ChX,DirX,ChY,DirY);
				}
			}
		}
		else
		{
			if(m_iPVMove==IDC_RADIO_PMOVE)
			{
				...
				{
					m_cDll.fast_pmove2(ChX,StepX,ChY,StepY);
				}
			}
			else
			{
				...
				{
					m_cDll.fast_vmove2(ChX,DirX,ChY,DirY);
				}
			}
		}
	}
	else	//直线插补运动,m_iSelect=1
	{
		StepX=atoi(m_sLXStep);	StepY=atoi(m_sLYStep);
		ChX=atoi(m_sLXAxes);	ChY=atoi(m_sLYAxes);

		m_cDll.set_vector_conspeed(atof(m_sLLowSpeed));
		m_cDll.set_vector_profile(atof(m_sLLowSpeed),atof(m_sLHighSpeed),atof(m_sLAccel),atof(m_sLAccel));

		ChX=atoi(m_sLXAxes);	ChY=atoi(m_sLYAxes);

		if(m_iLineConFast==IDC_RADIO_LINE_CON)
		{
			m_cDll.con_line2(ChX,StepX,ChY,StepY);
		}
		else
		{
			m_cDll.fast_line2(ChX,StepX,ChY,StepY);
		}
	}
}
...
void CDemo1Dlg::OnBtnSuddenstop() 
{
	...
	m_cDll.sudden_stop(atoi(strx));
	m_cDll.sudden_stop(atoi(stry));
}
...
void CDemo1Dlg::OnBtnDescendstop() 
{
	...
	m_cDll.decel_stop(atoi(strx));
	m_cDll.decel_stop(atoi(stry));
}
...
//	int iErr[10],i;
	...
	m_cDll.get_abs_pos(atoi(strx),&dbPosX);	//获取X轴位置
	m_cDll.get_abs_pos(atoi(stry),&dbPosY);	//获取Y轴位置
	PosX = dbPosX;
	PosY = dbPosY;
	m_cG.LineToPos(PosX,PosY);			//画线
	
//	for(i=1;i<11;i++)
//		iErr[i-1]=m_cDll.get_err(i);		//读取错误代码
...
void CDemo1Dlg::OnBtnReset() 
{
	...
	m_cDll.reset_pos(atoi(strx));
	m_cDll.reset_pos(atoi(stry));
